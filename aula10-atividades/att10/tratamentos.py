...
class CadastroTratamentos:
    ...
    def criar_labels(self):
        Label(self.tela, text="Cadastro de Tratamentos", font=("Arial", 16, "bold"), bg="#6daed1").place(x=130, y=15)
        
        Label(self.tela, text="Código:", bg="#6daed1").place(x=130, y=70)
        Label(self.tela, text="Médico responsável:", bg="#6daed1").place(x=130, y=110)
        Label(self.tela, text="Nome Paciente:", bg="#6daed1").place(x=130, y=150)
        Label(self.tela, text="Tipo de tratamento:", bg="#6daed1").place(x=130, y=190)

    def criar_campos(self):
        self.txt_codigo = Entry(self.tela, width=10)
        self.txt_medico = Entry(self.tela, width=50)
        self.txt_nomeP = Entry(self.tela, width=50)
        self.txt_tratamento = Entry(self.tela, width=15)

        self.txt_codigo.place(x=240, y=70)
        self.txt_medico.place(x=240, y=110)
        self.txt_nomeP.place(x=240, y=150)
        self.txt_tratamento.place(x=240, y=190)
    ...
    def limpar(self):
        ...
        self.txt_tratamento.delete(0, END)

    def dados(self):
        return {
            "codigo": self.txt_codigo.get(),
            "nome": self.txt_medico.get(),
            "pasciente": self.txt_nomeP.get(),
            "data": self.txt_tratamento.get(),
        }
    ...
    def atualizar(self):
        codigo = self.txt_codigo.get()
        if not codigo:
            messagebox.showwarning("Aviso", "Digite o código do tratamento que deseja alterar.")
            return
    ...
    def apagar(self):
        codigo = self.txt_codigo.get()
        if not codigo:
            messagebox.showwarning("Aviso", "Digite o código do tratamento que deseja excluir.")
            return
        ...
        if resultado.deleted_count > 0:
            ...
            messagebox.showwarning("Aviso", "Tratamento não encontrado para exclusão.")
    ...
    def consultar(self):
        ...
        if resultado:
            ...
            self.txt_tratamento.insert(0, resultado.get("data", ""))
        else:
